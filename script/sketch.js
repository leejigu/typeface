...
function getEyeEyebrowDist(face) {
  ...
  const lastKpLeftEye = face.leftEye.keypoints[face.leftEye.keypoints.length - 1];
  const lastKpLeftEyebrow = face.leftEyebrow.keypoints[face.leftEyebrow.keypoints.length - 1];

  // 오른쪽 눈과 오른쪽 눈썹의 마지막 키포인트
  const lastKpRightEye = face.rightEye.keypoints[face.rightEye.keypoints.length - 1];
  const lastKpRightEyebrow = face.rightEyebrow.keypoints[face.rightEyebrow.keypoints.length - 1];

  // 왼쪽 눈과 눈썹 사이의 거리
  const leftDist = dist(
    lastKpLeftEye.x, lastKpLeftEye.y, lastKpLeftEye.z,
    lastKpLeftEyebrow.x, lastKpLeftEyebrow.y, lastKpLeftEyebrow.z
  );

  // 오른쪽 눈과 눈썹 사이의 거리
  const rightDist = dist(
    lastKpRightEye.x, lastKpRightEye.y, lastKpRightEye.z,
    lastKpRightEyebrow.x, lastKpRightEyebrow.y, lastKpRightEyebrow.z
  );
